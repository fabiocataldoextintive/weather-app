...
5. Favorites: user toggles star on detail card → `favoriteCityToggled` → persisted favorites map; favorites list shows all entries; remove via list or star; click favorite row → `favoriteSelected` → fresh weather fetch.
6. Detail mode: `CurrentWeatherCardComponent` can toggle favorite → `favoriteCityToggled` → persisted favorites map.
