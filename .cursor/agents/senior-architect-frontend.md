...
**NgRx state management**. Save and stay updated this principal entities: 1.weather data of the last 3 recent cities consulted and their names (e.g. if the user consults the weather of the city `Córdoba, Argentina`, you must save it in both app state and localStorage a key-value pair object like: `recent-cities: { {city: "Córdoba, Argentina", weather: <here goes the full data obtained by the corresponding GET endpoint of the desired city's weather>} }`, then if the user continues consulting any other city, you must add a new object in the `recent-cities` key. In the case that you have 3 cities saved, you can rewrite the `recent-cities` object, doing the replacement from the last item added. When you have to save the data in the app state, the key must be in camelCase, like: `recentCities`. If the user consults a city that recently looked up for, search the data of it in both app state and local storage. Then, if you found an object with that city, searching in both `recent-cities` and `recentCities` state app and local storage locations, you must update and save the new `weather` key value in the corresponding city. In both app state and local storage, do not save an array of objects recent cities.), 2.names of the favorite cities saved by the user of the app (e.g. if the user consults the weather of the city `Córdoba, Argentina` and mark it as favorite, you must save in both app state and localStorage an object like: `favorite-cities: {"Córdoba, Argentina"}`. Then if the user continues consulting any other city, you must add a new string in the `favorites-cities` key. When you have to save the data in the app state, the key must be in camelCase, like: `favoritesCities`. If the user consults a city that recently looked up for, search the data of it in both app state and local storage.  Then, if you found an object with that city, searching in both `favorites-cities` and `favoritesCities` state app and local storage locations, you do not have to add a new item with the name of the city, because it will generate an repeated item.
In the app state and in local storage, do not save an array of strings.), 3.last visualization mode chosen by the user of the app (e.g. if the user selects `table`, you must save in both app state and localStorage a key-value pair object like: `visualization-mode: "table"`, otherwise `visualization-mode: "detailed"`. You have to search in the `favorites-cities` key value object app state or `favoritesCities`  key value object local storage. If the new visualization mode string is different than the saved one, you must replace it. Otherwise, do nothing. When you have to save the data in the app state, the key must be in camelCase, like: `visualization-mode`.).
